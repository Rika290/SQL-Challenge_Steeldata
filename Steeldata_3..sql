...
select avg(age) from customers where can_email='yes';
-- 4. How many orders were made by customers aged 30 or older?
select count(order_id) from orders
join customers using(customer_id)
where age>=30;
-- 5. What is the total revenue generated by each product category?
select category,sum(price)as total from products group by (1);
-- 6. What is the average price of products in the 'food' category?
select avg(price) from products group by category having category='food';
-- 7. How many orders were made in each sales channel (sales_channel column) in the orders table?
select sales_channel,count(order_id)as orders from orders group by (1);
-- 8.What is the date of the latest order made by a customer who can receive marketing emails?
select date_shop from orders o join customers c using(customer_id) where c.can_email='yes' order by (1) desc limit 1;
-- 9. What is the name of the country with the highest number of orders?
select c.country_name from country c join orders o using(country_id) 
group by c.country_name order by count(o.order_id) desc limit 1;
-- 10. What is the average age of customers who made orders in the 'vitamins' product category?
select avg(c.age)as average_age 
from customers c join orders o using(customer_id) join baskets b on b.order_id=o.order_id join products p on p.product_id=b.product_id
where p.category like 'vit%';
-----
select * from country;
